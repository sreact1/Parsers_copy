...
def Stop_Stemmer(vect):
    out1 = [item for item in vect if not item in stop]
    out2 = [stemmer.stem(item) for item in out1]
    return(out2)
...
# Внимание! В стеммер идёт на вход вектор из слов, а в Мар вектор из векторов из слов.
...
for j in range(4):
    ...
    np.save('data_stem/'+str(j)+'_error.npy',np.array(itog[0]))
    np.save('data_stem/'+str(j)+'_titles.npy',np.array(itog[1]))
    np.save('data_stem/'+str(j)+'_words.npy',np.array(itog[2]))
